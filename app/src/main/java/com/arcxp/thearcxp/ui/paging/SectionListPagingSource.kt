...
class SectionListPagingSource(val id: String, val pageSize: Int) : PagingSource<Int, ArcXPCollection>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ArcXPCollection> =
        withContext(Dispatchers.IO) {
            return@withContext try {
                ...
                when (val response =
                    ArcXPMobileSDK.contentManager().getCollectionSuspend(id = id, from = page)) {
                    is Success -> {
