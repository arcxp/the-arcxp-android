...
class CollectionPagingSource(val id: String, val pageSize: Int) : PagingSource<Int, ArcXPContentElement>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ArcXPContentElement> =
        withContext(Dispatchers.IO) {
            return@withContext try {
                ...
                when (val response =
                    ArcXPMobileSDK.contentManager().getCollectionSuspend(collectionAlias = id, from = page)) {
                    is Success -> {
